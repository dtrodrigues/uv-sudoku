...
"""
            os.makedirs(f"packages/{BASE_NAME}_{row+1}{col+1}_{digit}", exist_ok=True)
            with open(f"packages/{BASE_NAME}_{row+1}{col+1}_{digit}/pyproject.toml", "w") as f:
                f.write(tomlval)

            os.makedirs(f"packages/{BASE_NAME}_{row+1}{col+1}_{digit}/src/{BASE_NAME}_{row+1}{col+1}", exist_ok=True)
            with open(f"packages/{BASE_NAME}_{row+1}{col+1}_{digit}/src/{BASE_NAME}_{row+1}{col+1}/__init__.py", "w") as f:
                pass

            subprocess.run(["uv", "build", "--wheel", "-o", "../../wheels/"], cwd=f"packages/{BASE_NAME}_{row+1}{col+1}_{digit}", capture_output=True, text=True)
